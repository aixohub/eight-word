...
class Obb:
 ...
 rmc:new Array('初一','初二','初三','初四','初五','初六','初七','初八','初九','初十','十一','十二','十三','十四','十五','十六','十七','十八','十九','二十','廿一','廿二','廿三','廿四','廿五','廿六','廿七','廿八','廿九','三十','卅一'),
        ...
        self.yxmc = ["朔", "上弦", "望", "下弦"]
        self.jqmc = ['冬至', '小寒', '大寒', '立春', '雨水', '惊蛰', '春分', '清明', '谷雨', '立夏', '小满', '芒种',
                     '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪']
        # 月名称, 建寅
        self.ymc = ['十一', '十二', '正', '二', '三', '四', '五', '六', '七', '八', '九', '十']

        self.rmc = ['初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十', '十一', '十二',
    ...
    def get_NH(self, y):
        ...
        while i < len(ob):
            j = ob[i]
            if y < int(j) or y >= int(j) + int(ob[i + 1]):
                continue
            c = ob[i + 6] + (y - j + 1 + ob[i + 2]) + '年'
            # i为年号元年,i+3朝代,i+4朝号,i+5皇帝,i+6年号
            s += (' ' if s else '') + '[' + ob[i + 3] + ']' + ob[i + 4] + ' ' + ob[i + 5] + ' ' + c
        return s

    def get_day_name(self, u, r):
        m0 = ('0' if u.m < 10 else '') + u.m

        d0 = ('0' if u.d < 10 else '') + u.d
    ...
    def getDayName(self, u, r):  # 计算农历节日
        # 按农历日期查找重量点节假日
        d = u.Lmc + ('月' if u.Lmc.length < 2 else '') + u.Ldc
        if u.Lleap != '闰':
            if d == '正月初一':
                r.A += '春节 ',
                r.Fjia = 1  # Fjia(放假)
        ...
        if u.cur_dz >= 0 & u.cur_dz < 81:  # 数九
            w = self.numCn[math.floor(u.cur_dz / 9) + 1]
            if (u.cur_dz % 9 == 0):
                r.B += '『' + w + '九』 '
            else:
...
class SSQ:
    ...
    def __init__(self):
        self.suoKB = [1457698.231017, 29.53067166,  # -721-12-17 h=0.00032 古历·春秋
        ...
        self.qiKB = [1640650.479938, 15.21842500,  # -221-11-09 h=0.01709 古历·秦汉
        ...
        self.ZQ = []
        self.zhong_qi_pe1 = None
        self.zhong_qi_pe2 = None
        # 合朔表
        self.HS = []
        # 各月大小
        self.dx = []
    ...
    def qi_low(self, W):
        ...
        最大误差小于30分钟，平均5分
        """
        t, L, v = 628.3319653318, 628.3319653318, 628.3319653318
        t = (W - 4.895062166) / v  # 第一次估算, 误差2天以内
        # 第二次估算, 误差2小时以内
        t -= (53 * t * t + 334116 * math.cos(4.67 + 628.307585 * t) + 2061 * math.cos(
            2.678 + 628.3076 * t) * t) / v / 10000000
        # 平黄经 // 地球椭圆轨道级数展开 // 一次泊松项  // 光行差与章动修正
